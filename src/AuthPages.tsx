...
export function AccountPage() {
  ...
  const [activityTab, setActivityTab] = useState<"credits" | "payments">("credits");
    ...
    <Page themed account>
      <main className="account-page account-redesign">
        ...
        <section className="account-lower-grid">
          <Card className="account-section account-activity" aria-labelledby="activity-title"><header><div><span className="section-kicker">History</span><h2 id="activity-title">Recent activity</h2></div>{data && (activityTab === "credits" ? data.transactions.length : data.payments.length) > 8 && <Button variant="ghost" onClick={() => setActivityExpanded((value) => !value)}>{activityExpanded ? "Show less" : "See more"}</Button>}</header><div className="account-activity-tabs" role="tablist" aria-label="Recent activity type"><button type="button" role="tab" aria-selected={activityTab === "credits"} className={activityTab === "credits" ? "active" : ""} onClick={() => { setActivityTab("credits"); setActivityExpanded(false); }}><CreditCard size={16} />Credit activity</button><button type="button" role="tab" aria-selected={activityTab === "payments"} className={activityTab === "payments" ? "active" : ""} onClick={() => { setActivityTab("payments"); setActivityExpanded(false); }}><WalletCards size={16} />Payments</button></div><div className="account-activity-grid">{activityTab === "credits" ? <div className="transaction-list" role="tabpanel"><header><CreditCard size={17} /><h3>Credit activity</h3></header>{!data && <p>Loading credit activity...</p>}{data?.transactions.length === 0 && <p>No credit activity yet.</p>}{data?.transactions.slice(0, activityExpanded ? undefined : 8).map((transaction) => <div key={transaction.id}><span><b>{transaction.feature.replaceAll("_", " ")}</b><small>{transaction.type} · {transaction.bucket}</small></span><strong className={transaction.amount > 0 ? "positive" : ""}>{transaction.amount > 0 ? "+" : ""}{transaction.amount}</strong><time dateTime={transaction.created_at}>{new Date(transaction.created_at).toLocaleDateString()}</time></div>)}</div> : <div className="transaction-list payment-list" role="tabpanel"><header><WalletCards size={17} /><h3>Payments</h3></header>{!data && <p>Loading payments...</p>}{data?.payments.length === 0 && <p>No payment activity yet.</p>}{data?.payments.slice(0, activityExpanded ? undefined : 8).map((payment) => <div key={payment.id}><span><b>{payment.product_type === "subscription" ? "Stranerd Plus" : "100-credit pack"}</b><small>{payment.status}</small></span><strong>{formatMoney(payment.amount_minor, payment.currency)}</strong><time dateTime={payment.created_at}>{new Date(payment.created_at).toLocaleDateString()}</time></div>)}</div>}</div></Card>
