...
function PricingPage() {
  ...
  return <Page><main className="public-page"><header className="public-title"><h1>Learn freely. Use AI when it helps.</h1><p>Core anatomy models, guided Labs, free dissection, verified assessments, and default flashcards remain available without AI credits.</p></header><section className="pricing-grid"><Card><span>Free</span><h2>{nigeria ? 'NGN 0' : '$0'}</h2><p>Explore the complete anatomy learning workspace.</p><ul><li><Check size={15} />20 one-time signup credits</li><li><Check size={15} />All anatomy models and Labs</li><li><Check size={15} />Verified assessments and flashcards</li></ul><Button variant="outline" asChild><a href="/app">Start learning</a></Button></Card><Card className="featured"><span>Stranerd Plus</span><h2>{nigeria ? 'NGN 2,500' : '$5'} <small>/ month</small></h2><p>A monthly AI allowance for regular study.</p><ul><li><Check size={15} />500 credits each billing cycle</li><li><Check size={15} />{nigeria ? 'Secure NGN checkout' : 'USD card checkout'}</li><li><Check size={15} />Cancel with access through the period</li></ul><div className="billing-actions">{!options && <Button disabled>Loading payment options...</Button>}{options?.unavailable && <Button disabled>Payment options unavailable</Button>}{subscriptionRails.includes('ngn') && <BillingButton className="public-cta" productId="subscription" rail="ngn">Pay NGN 2,500</BillingButton>}{subscriptionRails.includes('usd_card') && <BillingButton className="public-cta" productId="subscription" rail="usd_card">Pay $5 by card</BillingButton>}</div></Card><Card><span>Credit pack</span><h2>{nigeria ? 'NGN 500' : '$2'}</h2><p>Add 100 non-expiring credits without a subscription.</p><ul><li><Check size={15} />100 purchased credits</li><li><Check size={15} />Stablecoin checkout available</li><li><Check size={15} />Buy packs repeatedly</li></ul><div className="billing-actions">{!options && <Button disabled>Loading payment options...</Button>}{options?.unavailable && <Button disabled>Payment options unavailable</Button>}{packRails.includes('ngn') && <BillingButton productId="payg_100" rail="ngn">Pay NGN 500</BillingButton>}{packRails.includes('usd_card') && <BillingButton productId="payg_100" rail="usd_card">Pay $2 by card</BillingButton>}{packRails.includes('stablecoin') && <BillingButton productId="payg_100" rail="stablecoin">Pay $2 in stablecoin</BillingButton>}</div></Card></section></main></Page>
}
