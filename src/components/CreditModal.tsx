...
export function CreditModal({ prompt, balance, onClose }: Props) {
  return <Dialog open={Boolean(prompt)} onOpenChange={(open) => { if (!open) onClose() }}>
    {prompt && <DialogContent className="credit-modal sm:max-w-md">
      ...
      <div className="credit-modal-actions"><BillingButton productId="payg_100">Buy 100 credits</BillingButton><Button variant="outline" asChild><a href="/pricing">View options<ArrowRight size={15} /></a></Button></div>
