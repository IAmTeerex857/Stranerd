...
export function LibraryView({ onAssessment, onFlashcards, onGeneratedDeck, onUnlockDeck, onReportDeck, generatedDecks, generatedError, completedQuizIds, flashcardProgressByDeck }: LibraryProps) {
  ...
  const renderGenerated = (deck: GeneratedDeckSummary) => {
    ...
    return <Card key={deck.id} className="library-resource-card generated-deck-card"><header><span className="resource-icon ai"><Sparkles /></span><Badge variant="secondary">AI deck</Badge></header><div className="resource-card-copy"><h3>{deck.title}</h3></div>{progress && <Progress value={(progress.reviewed / progress.total) * 100} />}<footer><span>{deck.owner ? deck.visibility : deck.unlocked ? `${progress?.reviewed ?? 0} of ${progress?.total ?? 15} reviewed` : 'Unlock once'}</span><div>{!deck.owner && <Button variant="ghost" size="sm" onClick={() => onReportDeck(deck)}>Report</Button>}{deck.owner || deck.unlocked ? <Button size="sm" onClick={() => onGeneratedDeck(deck)}>Study<ArrowRight /></Button> : <Button size="sm" onClick={() => onUnlockDeck(deck)}>Unlock · 5 credits</Button>}</div></footer></Card>
  }
