...
export default function MaterialsView(props: Props) {
  ...
  return <section className="materials-catalog" aria-labelledby="materials-heading"><div className="materials-catalog-heading"><div><h2 id="materials-heading">{copy.heading}</h2><p>{copy.detail}</p></div><label className="library-search"><span className="sr-only">Search subjects</span><Search aria-hidden="true" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search subjects" /></label></div>{filtered.length ? <div className="materials-grid">{filtered.map((entry) => { const count = mode === 'notes' ? entry.counts.sections : mode === 'flashcards' ? entry.counts.flashcards : entry.counts.questions; const unit = mode === 'notes' ? 'sections' : mode === 'flashcards' ? 'cards' : 'questions'; return <Card key={entry.id} className="material-subject-card"><header><span className="resource-icon material" aria-hidden="true"><copy.Icon /></span><span className="resource-status">{count} {unit}</span></header><div><h3>{entry.title}</h3><p>{mode === 'notes' ? 'Course notes' : mode === 'flashcards' ? 'Recall deck' : 'Verified practice test'}</p></div><footer><Button size="sm" onClick={() => selectSubject(entry)} aria-label={`${mode === 'notes' ? 'Read' : mode === 'flashcards' ? 'Study' : 'Start'} ${entry.title}`}>{mode === 'notes' ? 'Read' : mode === 'flashcards' ? 'Study' : 'Start'}<ArrowRight /></Button></footer></Card> })}</div> : <div className="empty-state">No subjects match “{query}”.</div>}</section>
}
...
function SubjectMaterials({ mode, subject, progress, onCardGrade, onNoteContext, onNoteRequest, onBack, ...learning }: LearningProps & { mode: LibraryContentMode; subject: MaterialSubject; progress?: FlashcardDeckProgress; onCardGrade: (cardId: string, grade: FlashcardGrade) => void; onNoteContext: Props['onNoteContext']; onNoteRequest: Props['onNoteRequest']; onBack: () => void }) {
  ...
  return <section className={`subject-materials subject-materials-${mode}`} aria-labelledby="subject-materials-heading"><header className="subject-materials-header"><Button variant="ghost" className="library-back" onClick={onBack}><ArrowLeft />All {mode === 'practice' ? 'practice tests' : mode}</Button><div><span className="page-kicker">{mode === 'practice' ? 'Practice test' : mode}</span><h2 id="subject-materials-heading">{subject.title}</h2></div></header>{mode === 'notes' && <MaterialNotes subject={subject} onNoteContext={onNoteContext} onNoteRequest={onNoteRequest} />}{mode === 'flashcards' && <MaterialCards subject={subject} progress={progress} onGrade={onCardGrade} {...learning} />}{mode === 'practice' && <MaterialPractice subject={subject} {...learning} />}</section>
}
...
function MaterialNotes({ subject, onNoteContext, onNoteRequest }: { subject: MaterialSubject; onNoteContext: Props['onNoteContext']; onNoteRequest: Props['onNoteRequest'] }) {
  ...
  return <div className="material-notes-layout"><aside className="material-section-list" aria-label={`${subject.title} sections`}><div className="material-section-title">{subject.title}<span>Sections</span></div><nav aria-label="Note sections">{sections.map((entry) => <button type="button" key={entry.id} className={selectedId === entry.id ? 'active' : ''} onClick={() => selectSection(entry.id)} aria-current={selectedId === entry.id ? 'page' : undefined}><span>{entry.ordinal + 1}</span><b>{entry.title}</b></button>)}</nav></aside><article className="material-note">{section && context ? <><header><span>{subject.title} · pages {section.pageStart}{section.pageEnd !== section.pageStart ? `–${section.pageEnd}` : ''}</span><h1>{section.title}</h1></header><MaterialMarkdown markdown={section.content} mnemonics={mnemonicMap} noteContext={context} onSelection={(selectedText) => onNoteContext({ ...context, selectedText })} onRequest={onNoteRequest} /><footer className="material-selection-help">Select any passage to copy it or ask Mentor to explain.</footer></> : <Loading />}</article></div>
}
