  ...
  return <section className="content-view flashcards-view anim">
    ...
    <Card className={`flashcard-stage ${flipped ? 'flipped' : ''} ${reducedMotion ? 'reduced-motion' : ''}`} tabIndex={0} role="button" aria-label={`${flipped ? 'Answer' : 'Question'} side. ${flipped ? 'Tap to return to the question.' : 'Tap to reveal the answer.'}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={() => { pointer.current = undefined }} onKeyDown={onKeyDown}>
      <div className="flashcard-inner">
        <div className={`flashcard-face flashcard-front ${card.front.diagram ? '' : 'text-only'}`} aria-hidden={flipped}><div className="flashcard-copy"><span>Question</span><h2>{card.front.heading}</h2><p>{card.front.body}</p></div>{card.front.diagram ? <FlashcardDiagram key={`${card.id}:${flipped}:${card.front.diagram.variantId}`} diagram={card.front.diagram} /> : <div className="flashcard-text-visual"><RotateCcw size={24} /><span>Think before revealing</span></div>}<small>{card.front.diagram ? 'Drag the model to inspect it.' : 'Answer from memory before revealing.'}</small></div>
