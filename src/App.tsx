...
import { BookOpen, Bot, ChevronRight, CircuitBoard, ClipboardList, Library, Menu, Moon, Network, NotebookPen, PanelLeftClose, PanelLeftOpen, Search, Star, Sun, X } from 'lucide-react'
...
export default function App() {
  ...
  async function selectHotspot(hotspot: Hotspot, multi: boolean, explain = true) {
    ...
    setMessages((current) => [...current, contextMessage, { id: replyId, role: 'mentor', text: 'Preparing a grounded response…', pending: true }])
    setTyping(true)
    try {
      ...
      setMessages((current) => current.map((message) => message.id === replyId ? { ...message, text: explanation, pending: false } : message))
      if (window.innerWidth <= 760) setMentorOpen(true)
    } finally {
      setTyping(false)
    }
  }
  ...
  async function explainDissectionAction(context: DissectionActionContext) {
    ...
    setTyping(true)
    const replyId = crypto.randomUUID()
    setMessages((current) => [...current, { id: replyId, role: 'mentor', text: 'Preparing a grounded response…', pending: true }])
    try {
