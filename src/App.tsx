...
function Brand({ onExplore }: { onExplore: () => void }) {
  return <a className="brand" href="/app" onClick={(event) => { event.preventDefault(); onExplore() }}><img className="brand-full" src={logoUrl} alt="Stranerd" /><img className="brand-icon" src={iconUrl} alt="" /><span>Interactive anatomy</span></a>
}
...
export default function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${!desktopMentorOpen ? 'mentor-hidden' : ''}`}>
    <header className="mobile-head">
      ...
      <a className="mobile-credits" href="/account" aria-label={`${creditBalance} credits`}><Zap size={14} /><b>{creditBalance}</b></a>
    ...
    {assistantAvailable && <VoiceLayer mode={voiceMode} modelId={model.id} context={voiceContext} onAction={handleVoiceAction} onInsufficientCredits={() => setCreditPrompt({ action: 'A five-minute Voice session', required: 10 })}>{(voice) => {
      const voiceActive = Boolean(voice.status && !['ended', 'error'].includes(voice.status))
      return <><MentorPanel model={model} selectedHotspot={selectedHotspot} actionHistory={persisted.dissectionActionsByModel[model.id] ?? []} messages={messages} typing={typing} onMessages={setMessages} onTyping={setTyping} mobileOpen={mentorOpen} onMobileClose={() => setMentorOpen(false)} onDesktopClose={() => setDesktopMentorOpen(false)} onInsufficientCredits={() => setCreditPrompt({ action: 'A Nerd Bot response', required: 1 })} noteContext={activeNoteContext} noteRequest={noteMentorRequest} voice={voice} /><aside className={`mentor-rail voice-${voice.muted ? 'muted' : voice.status ?? 'idle'}`} aria-label="Collapsed Nerd Bot"><button onClick={() => setDesktopMentorOpen(true)} aria-label={`Open Nerd Bot${voice.status ? `, Voice ${voice.muted ? 'muted' : voice.status}` : ''}`} aria-controls="stranerd-mentor">{voiceActive ? <VoiceSessionCuff compact voice={voice} /> : <><Bot size={21} />{typing && <i />}</>}</button></aside><button className={`mobile-mentor-button voice-${voice.muted ? 'muted' : voice.status ?? 'idle'}`} onClick={() => setMentorOpen(true)} aria-label={`Open Nerd Bot${voice.status ? `, Voice ${voice.muted ? 'muted' : voice.status}` : ''}`} aria-expanded={mentorOpen} aria-controls="stranerd-mentor" data-voice-active={voiceActive ? 'true' : undefined}>{voiceActive ? <VoiceSessionCuff voice={voice} /> : <><Bot size={25} />{typing && <i />}</>}</button></>
    }}</VoiceLayer>}
